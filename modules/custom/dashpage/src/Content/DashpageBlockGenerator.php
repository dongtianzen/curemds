...
class DashpageBlockGenerator extends ControllerBase {
  ...
  public function contentRenderCharts() {
    ...
    $output .= '<div class="content-render-chart-wrapper">';
      ...
      // $output .= '<div ng-if="block.middle.middleMiddle.middleMiddleMiddle.chartData.length">';
    ...
    $output .= '<div id="popUpParent" class="content-render-table-wrapper">';
      ...
      $output .= '<div class="{{block.middle.middleMiddle.middleMiddleMiddleClass}}">';
        $output .= '<div data-ng-controller="AngularDataTables" class="row margin-0 margin-top-16">';
          $output .= '<div data-ng-controller="PopUpController">';
        ...
        $output .= '<div data-ng-controller="AngularDataTables" class="row margin-0 margin-top-16">';
          $output .= '<table id="mildder-table" datatable="ng" dt-options="dtOptionsMildderTable" dt-instance="dtIntanceCallback" class="stripe responsive no-wrap">';
            ...
            $output .= '<tbody>';
              $output .= '<tr data-ng-repeat="tableRow in block.middle.middleMiddle.middleMiddleMiddle.value.tbody">';
                $output .= '<td data-ng-repeat="tableCell in tableRow track by $index">';
                  $output .= '<span ng-bind-html="$sce.trustAsHtml(tableCell)">';
                    $output .= '{{tableCell}}';
                  $output .= '</span>';
                $output .= '</td>';
              $output .= '</tr>';
  ...
  public function contentRenderPhpTable() {
    ...
    if (isset($path_args[4])) {
      if ($path_args[1] == 'dashpage' && $path_args[2] == 'webinar' && strlen($path_args[4]) > 0) {
        if (2 > 1) {
          $ManageinfoTableContent = new ManageinfoTableContent();
          // $table_content = $ManageinfoTableContent->tableContentData('meeting', 'all');
          $table_content = $ManageinfoTableContent->tableContentData($path_args[2], $path_args[4]);

          $DashpageObjectContent = new DashpageObjectContent();
          $table_content = $DashpageObjectContent->tableWebinar();
        }
      }
    }
      ...
      $output .= '<md-tab ng-repeat="block in block.middle.value" label="{{block.title}}">';
        ...
        $output .= '<div ng-switch="block.type">';
          ...
          $output .= '<div ng-switch-when="commonTable" class="{{block.class}}">';
            $output .= $this->contentRenderTable();
          $output .= '</div>';
          $output .= '<div ng-switch-when="multiContainer" class="{{block.class}}">';
      ...
      $output .= '<div class="panel-body bg-ffffff padding-0">';
        $output .= '<div ng-if="block.middle.enable">';
          $output .= '<div class="{{block.blockClasses}}">';
            $output .= '<div ng-switch="block.type">';
              $output .= '<div ng-switch-when="multiContainer">';
                $output .= $this->contentRenderMultiContainer();
              $output .= '</div>';
              $output .= '<div ng-switch-when="chart">';
                $output .= $this->contentRenderCharts();
